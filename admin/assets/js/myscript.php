<script type="text/javascript">
    ...
    function previewWatermark() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                toDataURL('index.php?com=photo&act=preview-watermark&type=<?= $type ?>&position=' + data.position + '&img=' + data.image + '&watermark=' + data.path + '&upload=' + data.upload + '&opacity=' + data.data.opacity + '&per=' + data.data.per + '&small_per=' + data.data.small_per + '&min=' + data.data.min + '&max=' + data.data.max + "&t=" + data.time, function(dataUrl) {
    ...
    $(document).ready(function() {
        ...
        $('.sidebar .nav-link').each(function() {
            ...
            var currentType = currentURL.searchParams.get('type');
            var currentCom = currentURL.searchParams.get('com');

            var navURL = new URL(href, currentLink);
            var navType = navURL.searchParams.get('type');
            var navCom = navURL.searchParams.get('com');

            function isActive(ele) {
                // return ($(this).hasClass('active'));
                return currentCom == navCom && currentType == navType;
            }


            if (isActive()) {
                var parentNode = $(this).parents('.nav-item')
                if (parentNode.hasClass('has-treeview')) {
                    parentNode.addClass('menu-open');
                }
                parentNode.children('.nav-link').addClass('active');
                // return false
            }
        })
