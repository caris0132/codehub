...
if ($error == '') {
	...
	if ($username == '' && $password == '') {
		...
		/* Kiểm tra đăng nhập */
		$row = $d->rawQueryOne("select * from table_user where username = ? and hienthi>0", array($username));

		if ($row['id']) {
			if (($row['password'] == Helper::encrypt_password($config['website']['secret'], $password, $config['website']['salt']))) {
				$timenow = time();
				$id_user = $row['id'];
				$ip = Helper::getRealIPAddress();
				$token = md5(time());
