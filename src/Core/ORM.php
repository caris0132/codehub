...
{
    /**
     * Working instance of Database created earlier
     *
     * @var Database
     */
    private $db;
    ...
    public function __construct($data = null)
    {
        ...
        if (empty($this->dbTable))
            $this->dbTable = (new \ReflectionClass($this))->getShortName();
    ...
    {
        $this->db->where(Database::$prefix . $this->dbTable . '.' . $this->primaryKey, $id);
    ...
    private function join($objectName, $key = null, $joinType = 'LEFT', $primaryKey = null)
    {
        ...
        if (!$primaryKey)
            $primaryKey = Database::$prefix . $joinObj->dbTable . "." . $joinObj->primaryKey;

        if (!strchr($key, '.'))
            $joinStr = Database::$prefix . $this->dbTable . ".{$key} = " . $primaryKey;
        else
            $joinStr = Database::$prefix . "{$key} = " . $primaryKey;
     ...
     *
     * Provides magic access to private functions of the class and native public Database functions
     *
     * @param string $method
     * @param mixed $arg
     *
